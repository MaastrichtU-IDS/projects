...
function _default(program) {
  ...
  program.command('client:install:ios').description('Install the Expo client for iOS on the simulator').option('--latest', 'Ignore the local project version and install the latest available version of Expo client').helpGroup('client').asyncAction(async command => {
  ...
  program.command('client:install:android').description('Install the Expo client for Android on a connected device or emulator').option('--latest', 'Ignore the local project version and install the latest available version of Expo client').helpGroup('client').asyncAction(async command => {
    const forceLatest = !!command.latest;
    const currentSdkConfig = await ClientUpgradeUtils().getExpoSdkConfig(process.cwd());
    const currentSdkVersion = currentSdkConfig ? currentSdkConfig.sdkVersion : undefined;
    const sdkVersions = await _xdl().Versions.sdkVersionsAsync();
